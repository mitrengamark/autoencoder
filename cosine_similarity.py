...
def compute_cosine_similarity(groups):
    group_names = list(groups.keys())
    group_vectors = [
        np.mean(
            [
                np.load(os.path.join(save_directory, f + ".npy"))
                for f in groups[group]
                if os.path.exists(os.path.join(save_directory, f + ".npy"))
            ],
            axis=0,
        )
    ...
    return group_names, similarity_matrix


def plot_confusion_matrix(group_names, similarity_matrix):
...
groups = {
    f"group_{i+1}": basic_maneuvers_list[i] for i in range(len(basic_maneuvers_list))
}

group_names, similarity_matrix = compute_cosine_similarity(groups)
plot_confusion_matrix(group_names, similarity_matrix)
